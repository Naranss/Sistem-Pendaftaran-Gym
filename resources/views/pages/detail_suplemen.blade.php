        ...
        <div class="bg-gradient-to-b from-gray-800 to-gray-900 rounded-xl shadow-2xl overflow-hidden border border-gray-700 mb-8">
            <div class="flex flex-col lg:flex-row gap-8 p-8">
                ...
                <div class="w-full lg:w-2/3 space-y-6">
                    ...
                    <div class="relative overflow-hidden rounded-xl" x-data="{ activeIndex: 0, showModal: false }">
                        @if(isset($suplemen->gambarSuplemen) && count($suplemen->gambarSuplemen) > 0)
                        <div class="flex transition-transform duration-500 cursor-pointer"
                            @click="showModal = true"
                            :style="{ transform: `translateX(-${activeIndex * 100}%)` }">
                            @foreach ($suplemen->gambarSuplemen as $gambar)
                            <img src="{{ asset($gambar->path) }}" class="w-full h-96 object-cover flex-shrink-0" alt="{{ $gambar->img_alt ?? $suplemen->nama_suplemen }}" />
                            @endforeach
                        </div>
                        ...
                        @endif
                        @else
                        <div class="w-full h-96 bg-gradient-to-br from-gray-700 to-gray-900 flex items-center justify-center rounded-lg">
                        ...
                        <div class="fixed inset-0 bg-black/95 z-50 flex items-center justify-center p-4"
                            ...
                            <div class="flex items-center justify-center gap-4 w-full h-full">
                                @if(count($suplemen->gambarSuplemen) > 1)
                                <button @click="activeIndex = activeIndex > 0 ? activeIndex - 1 : {{ count($suplemen->gambarSuplemen) - 1 }}"
                                    class="text-white hover:text-gray-300 transition p-2 hover:bg-white/10 rounded-full z-20">
                                    <svg class="w-8 h-8" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 19l-7-7 7-7" />
                                    </svg>
                                </button>
                                ...
                                <div class="flex transition-transform duration-500 max-w-4xl max-h-[90vh]"
                                    :style="{ transform: `translateX(-${activeIndex * 100}%)` }">
                                    @foreach ($suplemen->gambarSuplemen as $gambar)
                                    <img
