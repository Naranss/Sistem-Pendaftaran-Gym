...
<main class="flex-grow min-h-screen bg-gradient-to-b from-gray-900 to-black py-8">
    <div class="container mx-auto px-6">
        ...
        <div class="flex justify-between items-center mb-6">
            <h1 class="text-3xl font-bold text-white">Kelola Suplemen</h1>
        ...
        {{-- Search Bar --}}
        <div class="mb-6">
            <form method="GET" action="{{ route('admin.suplemen') }}" class="flex gap-4">
                <div class="flex-1">
                    <input 
                        type="text" 
                        name="search" 
                        value="{{ request('search') }}"
                        placeholder="Cari suplemen..." 
                    ...
                    <tbody class="bg-gray-800 divide-y divide-gray-700">
                        @forelse ($suplemen as $item)
                            ...
                            {{ $item->nama_suplemen }}
                                ...
                                <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-300">
                                    <span class="px-2 py-1 rounded {{ $item->stok > 10 ? 'bg-green-900 text-green-300' : ($item->stok > 0 ? 'bg-yellow-900 text-yellow-300' : 'bg-red-900 text-red-300') }}">
                                        {{ $item->stok }}
                                    </span>
                                </td>
                                <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-300">
                                ...
                                <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-300">
                                    @php
                                        ...
                                        $isNearExpired = $tanggalKadaluarsa->diffInDays(now()) <= 30 && !$isExpired;
                                    @endphp
                                ...
                                <td class="px-6 py-4 whitespace-nowrap text-sm font-medium">
                                    <div class="flex items-center gap-3">
                                        ...
                                        <button 
                                            onclick="confirmDelete({{ $item->id }}, '{{ $item->nama_suplemen }}')"
                            ...
                            <tr>
                                <td colspan="7" class="px-6 py-8 text-center text-gray-400">
                                    <div class="flex flex-col items-center justify-center">
                                        ...
                                        <p>Tidak ada data suplemen ditemukan</p>
                                        @if(request('search'))
...
<div id="addModal" class="fixed inset-0 bg-black bg-opacity-50 hidden items-center justify-center z-50">
    <div class="bg-gray-800 rounded-lg p-6 w-full max-w-md mx-4">
        <div class="flex justify-between items-center mb-4">
            <h2 class="text-xl font-bold text-white">Tambah Suplemen</h2>
        ...
        <form id="addForm" method="POST" action="{{ route('admin.suplemen.store') }}" onsubmit="return validateAddForm()">
            ...
            <div class="space-y-4">
                <div>
                    <label class="block text-sm font-medium text-gray-300 mb-2">Nama Suplemen</label>
                ...
                <div>
                    <label class="block text-sm font-medium text-gray-300 mb-2">Tanggal Kadaluarsa</label>
...
<div id="editModal" class="fixed inset-0 bg-black bg-opacity-50 hidden items-center justify-center z-50">
    <div class="bg-gray-800 rounded-lg p-6 w-full max-w-md mx-4">
        <div class="flex justify-between items-center mb-4">
            <h2 class="text-xl font-bold text-white">Edit Suplemen</h2>
